    ...
    public boolean checkIfPasses(ServerPlayerEntity entity, ServerWorld serverWorld, StructureStart<?> structureStart, MutableBoundingBox box, BlockPos target, ActionType type, List<TranslationTextComponent> requirements) {
        if (box.isInside(target)) {
            ...
            if (isPerPlayer()) {
                ...
                if (!passes) {
                    requirements.add(new TranslationTextComponent(StructureWarden.MOD_ID + ".condition.time", String.format("%.2f", (double) timeLeft / 24000)));
                }
                return passes;
            } else {
                if (this.timeLeft == -1) {
                ...
                if (!passes) {
                    requirements.add(new TranslationTextComponent(StructureWarden.MOD_ID + ".condition.time", String.format("%.2f", (double) timeLeft / 24000D)));
                }
                return passes;
            }
        }
